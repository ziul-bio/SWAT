...
# ask luiz tomorrow
# what order to put supp 8 and 9 in with new models (by size?)
...
#results/lassoCV/DMS/DMS_all_multi_model_metadata.csv
...
label_C300M <- bquote(R^2 ~ ": " ~ .(sprintf("%.2f", r_squared_C300M)))
...
#label_C600M <- bquote(R^2 ~ ": " ~ .(sprintf("%.2f", r_squared_C600M)))
label_C600M <- bquote("Regression " ~ R^2 ~ ": " ~ .(sprintf("%.2f", r_squared_C600M)))
...
label_A120M <- bquote(R^2 ~ ": " ~ .(sprintf("%.2f", r_squared_A120M)))
...
label_A350M <- bquote(R^2 ~ ": " ~ .(sprintf("%.2f", r_squared_A350M)))
...
data_text <- data.frame(
  ...
  Model = c("ESM1v 650M", "ESM2 8M", "ESM2 35M", "ESM2 150M", "ESM2 650M","ESM2 3B", "ESM2 15B", "ESMC 300M", "ESMC 600M", "Amplify 120M", "Amplify 350M"),
  x = c(1150, 1200, 1200, 1200, 1200, 1200, 1200, 1200, 1120, 1200, 1200),
  y = c(.78, .78, .78, .78, .78, .78, .78, .78, .83, .78, .78)
)


# not sure what order to do this part in 
data_text$Model <- factor(data_text$Model, levels = c("ESM1v 650M", "ESM2 8M", "ESM2 35M", "ESM2 150M", "ESM2 650M","ESM2 3B", "ESM2 15B", "ESMC 300M", "ESMC 600M", "Amplify 120M", "Amplify 350M"))
...
plabel_C300M <- bquote(R^2 ~ ": " ~ .(sprintf("%.2f", pr_squared_C300M)))
...
#plabel_C600M <- bquote(R^2 ~ ": " ~ .(sprintf("%.2f", pr_squared_C600M)))
plabel_C600M <- bquote("Regression " ~ R^2 ~ ": " ~ .(sprintf("%.2f", pr_squared_C600M)))
...
plabel_A120M <- bquote(R^2 ~ ": " ~ .(sprintf("%.2f", pr_squared_A120M)))
...
plabel_A350M <- bquote(R^2 ~ ": " ~ .(sprintf("%.2f", pr_squared_A350M)))
...
prot_data_text <- data.frame(
  ...
  Model = c("ESM1v 650M", "ESM2 8M", "ESM2 35M", "ESM2 150M", "ESM2 650M","ESM2 3B", "ESM2 15B", "ESMC 300M", "ESMC 600M", "Amplify 120M", "Amplify 350M"),
  x = c(130, 180, 180, 180, 180, 180, 180, 180, 180, 180, 180),
  y = c(.78, .78, .78, .78, .78, .78, .78, .78, .83, .78, .78)
)

prot_data_text$Model <- factor(data_text$Model, levels = c("ESM1v 650M", "ESM2 8M", "ESM2 35M", "ESM2 150M", "ESM2 650M","ESM2 3B", "ESM2 15B","ESMC 300M", "ESMC 600M", "Amplify 120M", "Amplify 350M"))
...
grouped_DMS_sampling <- DMS_sampling %>%
  mutate(Dataset = case_when(
  Dataset == "BLAT_ECOLX_2015" ~ "BLAT ECOLX Singles",
...
grouped_DMS_sampling <- grouped_DMS_sampling %>%
  ...
  mutate(Dataset = factor(Dataset, levels = c("BLAT ECOLX Singles", "PABP Doubles", "HIS7 Multiple"))) %>%
...
# mutate(Dataset = case_when(
#   Dataset == "BLAT_ECOLX_2015" ~ "BLAT",
#   Dataset == "PABP_doubles" ~ "PABPDoubles",
#   Dataset == "HIS7" ~ "HIS7Multiple",
#   #TRUE ~ Dataset
#   )) %>%
#mutate(Dataset = factor(Dataset, levels = c("BLAT ECOLX 2015 Singles", "PABP Doubles", "HIS7 Multiple")))
