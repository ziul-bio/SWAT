...
```{r}
multi_mod_DMS <- read.csv(here("results/summary_DMS_all_multi_model.csv"))
...
```{r}
#grouped_150M_650M_15B

gradient_plot <- grouped_multi_mod_DMS %>%
...
#boxplot version xD
# 
# color_dict = c("esm1v 650M" = "#a1cded", "esm2 8M" = "#88b0d4", "esm2 35M" = "#7093bb", "esm2 150M" = "#5a78a3", "esm2 650M" = "#465d8a", "esm2 3B" = "#334371", "esm2 15B" = "#212b59")
#color_dict = c("esm1v 650M" = "#b3e0ff", "esm2 8M" = "#99c4e5", "esm2 35M" = "#80a8cc", "esm2 150M" = "#688eb2", "esm2 650M" = "#51749a", "esm2 3B" = "#3b5b82", "esm2 15B" = "#25436a")
...
DMS_multi_boxplot <- grouped_multi_mod_DMS %>%
  #filter(Compression_method %in% c('mean')) %>%
  ggplot(aes(
    x = Model, 
    y = mean_score, 
...
DMS_multi_boxplot
   ...
   # ggsave(DMS_multi_boxplot, 
   #       filename = "DMS_multi_boxplot.png",
   #        path = here('figs'),
   #        width = 11,
   #       #6
   #        height = 4,
...
exclude_data <- c("BRCA1 RING", "UBE4B singles", "BRCA1 BRCT", "POLG Sun2014", "BG505 env")
#  c('BRCA1_HUMAN_RING', 'UBE4B_MOUSE_Klevit2013_singles',
#    'BRCA1_HUMAN_BRCT', 'POLG_HCVJF_Sun2014', 'BG505_env_Bloom2018')
...
# need to make an order for the x axis
#excluded_multi_mod_DMS$Model <- factor(excluded_multi_mod_DMS$Model, levels = c("esm1v 650M", "esm2 8M", "esm2 35M", "esm2 150M", "esm2 650M", "esm2 3B", "esm2 15B"))
...
# need to make an order for the x axis
#small_multi_mod_DMS$Model <- factor(small_multi_mod_DMS$Model, levels = c("esm1v 650M", "esm2 8M", "esm2 35M", "esm2 150M", "esm2 650M", "esm2 3B", "esm2 15B"))
...
# bquote(atop("Slope:" ~ .(round(as.numeric(slope_8M), 2)), 
#                             R^2 ~ ":" ~ .(round(as.numeric(r_squared_8M), 2)) ~ "     "))


#ESM1v 650M
model_ESM1v_650M <- lm(mean_score ~ log(dataset_size), data = data_with_size, subset = Model == "esm1v 650M")
slope_ESM1v_650M <- coef(model_ESM1v_650M)[2]
r_squared_ESM1v_650M <- summary(model_ESM1v_650M)$r.squared
label_ESM1v_650M <- bquote(atop("Slope:" ~ .(sprintf("%.2f", as.numeric(slope_ESM1v_650M))), 
                         " " ~ R^2 ~ ":" ~ .(sprintf("%.2f", as.numeric(r_squared_ESM1v_650M))) ~ "     "))

# ESM2 8M
model_8M <- lm(mean_score ~ log(dataset_size), data = data_with_size, subset = Model == "esm2 8M")
slope_8M <- coef(model_8M)[2]
r_squared_8M <- summary(model_8M)$r.squared
label_8M <- bquote(atop("Slope:" ~ .(sprintf("%.2f", as.numeric(slope_8M))), 
                         " " ~ R^2 ~ ":" ~ .(sprintf("%.2f", as.numeric(r_squared_8M))) ~ "     "))

#ESM2 35M
model_35M <- lm(mean_score ~ log(dataset_size), data = data_with_size, subset = Model == "esm2 35M")
slope_35M <- coef(model_35M)[2]
r_squared_35M <- summary(model_35M)$r.squared
label_35M <- bquote(atop("Slope:" ~ .(sprintf("%.2f", as.numeric(slope_35M))), 
                         " " ~ R^2 ~ ":" ~ .(sprintf("%.2f", as.numeric(r_squared_35M))) ~ "     "))


#ESM2 150M
model_150M <- lm(mean_score ~ log(dataset_size), data = data_with_size, subset = Model == "esm2 150M")
slope_150M <- coef(model_150M)[2]
r_squared_150M <- summary(model_150M)$r.squared
label_150M <- bquote(atop("Slope:" ~ .(sprintf("%.2f", as.numeric(slope_150M))), 
                         " " ~ R^2 ~ ":" ~ .(sprintf("%.2f", as.numeric(r_squared_150M))) ~ "     "))

#ESM2 650M
model_650M <- lm(mean_score ~ log(dataset_size), data = data_with_size, subset = Model == "esm2 650M")
slope_650M <- coef(model_650M)[2]
r_squared_650M <- summary(model_650M)$r.squared
label_650M <- bquote(atop("Slope:" ~ .(sprintf("%.2f", as.numeric(slope_650M))), 
                         " " ~ R^2 ~ ":" ~ .(sprintf("%.2f", as.numeric(r_squared_650M))) ~ "     "))

#ESM2 3B
model_3B <- lm(mean_score ~ log(dataset_size), data = data_with_size, subset = Model == "esm2 3B")
slope_3B <- coef(model_3B)[2]
r_squared_3B <- summary(model_3B)$r.squared
label_3B <- bquote(atop("Slope:" ~ .(sprintf("%.2f", as.numeric(slope_3B))), 
                         " " ~ R^2 ~ ":" ~ .(sprintf("%.2f", as.numeric(r_squared_3B))) ~ "     "))

#ESM2 15B
model_15B <- lm(mean_score ~ log(dataset_size), data = data_with_size, subset = Model == "esm2 15B")
slope_15B <- coef(model_15B)[2]
r_squared_15B <- summary(model_15B)$r.squared
label_15B <- bquote(atop("Slope:" ~ .(sprintf("%.2f", as.numeric(slope_15B))), 
                         " " ~ R^2 ~ ":" ~ .(sprintf("%.2f", as.numeric(r_squared_15B))) ~ "     "))


data_text <- data.frame(
  #labels = c(label_150M, label_650M, label_15B),
  labels = I(list(label_ESM1v_650M, label_8M, label_35M, label_150M, label_650M, label_3B, label_15B)),
  Model = c("esm1v 650M", "esm2 8M", "esm2 35M", "esm2 150M", "esm2 650M","esm2 3B", "esm2 15B"),
  x = c(1050, 1050, 1050, 1050, 1050, 1050, 620),
  y = c(.8, .8, .8, .8, .8, .8, .8)
)

data_text$Model <- factor(data_text$Model, levels = c("esm1v 650M", "esm2 8M", "esm2 35M", "esm2 150M", "esm2 650M","esm2 3B", "esm2 15B"))
...
plot_param <- data_with_size %>%
  filter(Model != 'esm2 15B') %>%
    ggplot(aes(
      x = dataset_size, 
      y = mean_score,
      color = group
      )) +
    scale_x_continuous(limits = c(300, NA),
                       #limits = c(1000, NA),
  ...
  guides(color = guide_legend(title = NULL))  +
    ...
    geom_text(
        data = filter(data_text, Model != "esm2 15B"),
        mapping = aes(x = x, y = y, label = labels),
        color = "black",
        parse=TRUE,
        #hjust = 0,     # Adjust label alignment
    #vjust = 0
        )+
...
#plot_param <- plot_param + facet_grid(cols = vars(Model))
...
#ESM1v 650M
pmodel_ESM1v_650M <- lm(mean_score ~ log(prot_length), data = data_with_size, subset = Model == "esm1v 650M")
pslope_ESM1v_650M <- coef(pmodel_ESM1v_650M)[2]
pr_squared_ESM1v_650M <- summary(pmodel_ESM1v_650M)$r.squared
plabel_ESM1v_650M <- bquote(atop("Slope:" ~ .(sprintf("%.2f", as.numeric(pslope_ESM1v_650M))), 
                         " " ~ R^2 ~ ":" ~ .(sprintf("%.2f", as.numeric(pr_squared_ESM1v_650M))) ~ "     "))

# ESM2 8M
pmodel_8M <- lm(mean_score ~ log(prot_length), data = data_with_size, subset = Model == "esm2 8M")
pslope_8M <- coef(pmodel_8M)[2]
pr_squared_8M <- summary(pmodel_8M)$r.squared
plabel_8M <- bquote(atop("Slope:" ~ .(sprintf("%.2f", as.numeric(pslope_8M))), 
                         " " ~ R^2 ~ ":" ~ .(sprintf("%.2f", as.numeric(pr_squared_8M))) ~ "     "))

#ESM2 35M
pmodel_35M <- lm(mean_score ~ log(prot_length), data = data_with_size, subset = Model == "esm2 35M")
pslope_35M <- coef(pmodel_35M)[2]
pr_squared_35M <- summary(pmodel_35M)$r.squared
plabel_35M <- bquote(atop("Slope:" ~ .(sprintf("%.2f", as.numeric(pslope_35M))), 
                         " " ~ R^2 ~ ":" ~ .(sprintf("%.2f", as.numeric(pr_squared_35M))) ~ "     "))


#ESM2 150M
pmodel_150M <- lm(mean_score ~ log(prot_length), data = data_with_size, subset = Model == "esm2 150M")
pslope_150M <- coef(pmodel_150M)[2]
pr_squared_150M <- summary(pmodel_150M)$r.squared
plabel_150M <- bquote(atop("Slope:" ~ .(sprintf("%.2f", as.numeric(pslope_150M))), 
                         " " ~ R^2 ~ ":" ~ .(sprintf("%.2f", as.numeric(pr_squared_150M))) ~ "     "))

#ESM2 650M
pmodel_650M <- lm(mean_score ~ log(prot_length), data = data_with_size, subset = Model == "esm2 650M")
pslope_650M <- coef(pmodel_650M)[2]
pr_squared_650M <- summary(pmodel_650M)$r.squared
plabel_650M <- bquote(atop("Slope:" ~ .(sprintf("%.2f", as.numeric(pslope_650M))), 
                         " " ~ R^2 ~ ":" ~ .(sprintf("%.2f", as.numeric(pr_squared_650M))) ~ "     "))

#ESM2 3B
pmodel_3B <- lm(mean_score ~ log(prot_length), data = data_with_size, subset = Model == "esm2 3B")
pslope_3B <- coef(pmodel_3B)[2]
pr_squared_3B <- summary(pmodel_3B)$r.squared
plabel_3B <- bquote(atop("Slope:" ~ .(sprintf("%.2f", as.numeric(pslope_3B))), 
                         " " ~ R^2 ~ ":" ~ .(sprintf("%.2f", as.numeric(pr_squared_3B))) ~ "     "))

#ESM2 15B
pmodel_15B <- lm(mean_score ~ log(prot_length), data = data_with_size, subset = Model == "esm2 15B")
pslope_15B <- coef(pmodel_15B)[2]
pr_squared_15B <- summary(pmodel_15B)$r.squared
plabel_15B <- bquote(atop("Slope:" ~ .(sprintf("%.2f", as.numeric(pslope_15B))), 
                         " " ~ R^2 ~ ":" ~ .(sprintf("%.2f", as.numeric(pr_squared_15B))) ~ "     "))


prot_data_text <- data.frame(
  #labels = c(label_150M, label_650M, label_15B),
  labels = I(list(plabel_ESM1v_650M, plabel_8M, plabel_35M, plabel_150M, plabel_650M, plabel_3B, plabel_15B)),
  Model = c("esm1v 650M", "esm2 8M", "esm2 35M", "esm2 150M", "esm2 650M","esm2 3B", "esm2 15B"),
  x = c(200, 300, 300, 300, 300, 300, 175),
  y = c(.8, .8, .8, .8, .8, .8, .8)
)

prot_data_text$Model <- factor(data_text$Model, levels = c("esm1v 650M", "esm2 8M", "esm2 35M", "esm2 150M", "esm2 650M","esm2 3B", "esm2 15B"))
...
sup9_prot_length <- data_with_size %>%
  ...
  guides(color = guide_legend(title = NULL))  +
    ...
    geom_text(
        data = filter(prot_data_text, Model != "esm2 15B"),
        mapping = aes(x = x, y = y, label = labels),
        color = "black",
        parse=TRUE,
        #hjust = 0,     # Adjust label alignment
...
```{r}
pisces_data <- read.csv(here('results/summary_PISCES_lassoCV_esm2_150M_all_methods.csv'))
#'Pisces_summary.csv'
...
```{r}
multi_mod_pisces <- read.csv(here("results/summary_PISCES_multi_model.csv"))

multi_mod_pisces <- multi_mod_pisces %>%
  mutate(Dataset = case_when(
    Dataset == "PCP_AAfreq_Ala" ~ "A Freq",
    Dataset == "PCP_AAfreq_Cys" ~ "C Freq",
    Dataset == "PCP_AAfreq_Leu" ~ "L Freq",
    Dataset == "PCP_Charge" ~ "Charge",
    Dataset == "PCP_Hydrophobicity" ~ "Hydrophobicity",
    Dataset == "PCP_Instability_index" ~ "Instability Index",
    Dataset == "PCP_Isoelectric_point" ~ "Isoelectric Point",
    Dataset == "PCP_length" ~ "Length",
    Dataset == "PCP_mW_kDa" ~ "mW kDa",
    Dataset == "SS_Coil" ~ "Coil",
    Dataset == "SS_E" ~ "SS E",
    Dataset == "SS_H" ~ "SS H",
    TRUE ~ Dataset
...
# need to make an order for the x axis
grouped_multi_mod_pisces$Model <- factor(grouped_multi_mod_pisces$Model, levels = c("esm1v 650M", "esm2 8M", "esm2 35M", "esm2 150M", "esm2 650M", "esm2 3B", "esm2 15B"))
...
gradient_pisces <- grouped_multi_mod_pisces %>%
...
pisces_multi_boxplot <- grouped_multi_mod_pisces %>%
  #filter(Compression_method %in% c('mean')) %>%
  ggplot(aes(
    x = Model, 
    y = mean_score, 
...
pisces_multi_boxplot
 
# ggsave(pisces_multi_boxplot, 
#       filename = "pisces_multi_boxplot.png",
#        path = here('figs'),
#        width = 11,
#       #6
#        height = 4,
...
```{r}
DMS_sampling <- read.csv(here('results/summary_DMS4_sampling.csv'))
...
grouped_DMS_sampling <- DMS_sampling %>%
  group_by(Dataset, Sample_size, Model) %>%
...
grouped_DMS_sampling <- grouped_DMS_sampling %>%
  mutate(Model = factor(Model, levels = c("esm1v 650M", "esm2 8M", "esm2 35M", "esm2 150M", "esm2 650M", "esm2 3B", "esm2 15B")))


grouped_DMS_sampling <- grouped_DMS_sampling %>%
  mutate(Dataset = case_when(
    Dataset == "BLAT_ECOLX_2015" ~ "BLAT ECOLX 2015 (singles)",
    Dataset == "PABP_doubles" ~ "PABP (doubles)",
    Dataset == "parEparD_all" ~ "parEparD (1-4)",
    Dataset == "HIS7_YEAST" ~ "HIS7 (1-28)"
    )) %>%
    mutate(Dataset = factor(Dataset, levels = c("BLAT ECOLX 2015 (singles)", "PABP (doubles)", "parEparD (1-4)", "HIS7 (1-28)"))) 
    
parepard_sample <- grouped_DMS_sampling %>%
  filter(Dataset %in% c("parEparD (1-4)"))

grouped_DMS_sampling <- grouped_DMS_sampling %>%
  filter(!Dataset %in% c("parEparD (1-4)"))

# grouped_DMS_sampling <- grouped_DMS_sampling %>%
#   mutate(label = dataset_labels[Dataset])
...
DMS_sample_R2_plot <- grouped_DMS_sampling %>%
  ...
  # geom_text(data = grouped_DMS_sampling, aes(x = 95, y = .8, label = label),
  #           inherit.aes = FALSE,
  #           hjust = 1.1, vjust = 1.1,
...
supp_parepard_sample_R2_plot <- parepard_sample %>%
  ggplot(aes(
      x = Sample_size, 
      y = mean_R2_score,
      color = Model
      )) +
  scale_x_continuous(limits = c(95, NA),trans = scales::log_trans(),
                     breaks = scales::log_breaks(),
                     labels = trans_format("log10", math_format(10^.x))
                       ) +
  ...
  labs(title = "parEparD (1-4)", x= "Sample Size", 
    y = expression(R^2), fill = ""
       ) +
  theme_minimal_grid(12, rel_small = 1)+
  theme(
  ...
  # ggsave(supp_parepard_sample_R2_plot, 
  #        filename = "supp_parepard_sample_R2_plot.png",
  #        path = here("figs"),
  #        width = 6.75,
  #       height = 5,
  #        dpi=600)

supp_parepard_sample_R2_plot
...
```{r}
sampling_plots <- plot_grid(DMS_sample_R2_plot, 
                       sample_coeff_plot,
                        labels = c('A', 'B'), 
                        label_size = 12, 
                        rel_widths = c(1, 1),
...
# ggsave(sampling_plots,
#        filename = "sampling_plots.png",
#        path = here("figs"),
#        width = 13.5,
#        height = 8,
#        dpi=600)
...
# supplementary figs 
# Supplementary Figure 3 Assembly #NOT UPDATED
```{r}
...
# Fig 3A multimod assembly
```{r}
fig3_multi_mod <- plot_grid(DMS_multi_boxplot, 
                            pisces_multi_boxplot, 
                            labels = c('A', 'B'), 
                            ncol = 1, 
                            rel_heights = c(1.1, 1))
