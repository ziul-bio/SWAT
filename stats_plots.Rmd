...
# Figure 5 #NOT UPDATED


```{r}
# Fig 5
#'results/lassoCV_DMS/summary_all_DMS_results_LassoCV_mean.csv'
#data_with_size <- read.csv(here('results/summary_DMS_all_multi_model.csv'))
data_with_size <- read.csv(here('results/summary_DMS_all_multi_model.csv'))
...
label_150M <- bquote(atop("Slope:" ~ .(round(as.numeric(slope_150M), 2)), 
                            R^2 ~ ":" ~ .(round(as.numeric(r_squared_150M), 2)) ~ "     "))
...
label_650M <- bquote(atop("Slope:" ~ .(round(as.numeric(slope_650M), 2)), 
                            R^2 ~ ":" ~ .(round(as.numeric(r_squared_650M), 2)) ~ "   "))
...
label_15B <- bquote(atop("Slope:" ~ .(round(as.numeric(slope_15B), 2)), 
                            R^2 ~ ":" ~ .(round(as.numeric(r_squared_15B), 2)) ~ "     "))


data_text <- data.frame(
  #labels = c(label_150M, label_650M, label_15B),
  labels = I(list(label_150M, label_650M, label_15B)),
